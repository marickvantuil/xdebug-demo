...
sed -i '/^;listen.mode = .*/alisten.mode = 0660' /etc/php/7.3/fpm/pool.d/www.conf
sed -i '/^listen.mode = .*/alisten.owner = www-data' /etc/php/7.3/fpm/pool.d/www.conf
sed -i '/^listen.mode = .*/alisten.group = www-data' /etc/php/7.3/fpm/pool.d/www.conf
...
mkdir -p /var/run/php7.3-fpm && chown -R www-data:www-data /var/run/php7.3-fpm
mkdir -p /var/log/php7.3-fpm && chown -R www-data:www-data /var/log/php7.3-fpm
